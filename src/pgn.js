...
var AllPGNLineCommentMatcher=new RegExp("\\:(.*?)(\r\n|\r|\n)","g");
var AllWhiteSpacePlaceHolderMatcher=new RegExp("\x03","g");
var AllLineSplitterMatcher=new RegExp("\r\n|\r|\n","g");
var MoveNumberMatcher=new RegExp("(^\\d*\\.+)|(^\\d+$)","");
var WhiteSpaceMatcher=new RegExp("[ ]+","");
var LineSplitterReplacer=new RegExp("\\r\\n|\\r","g");
var SymbolLengthSorted=["??","?!","!?","!!","?","!"];

export var ParseSinglePGN=function(PGNString)
{
...
var GetSymbolFromMove=function(MoveText)
{
    ...
    for (i=0;i<SymbolLengthSorted.length;i++)
    {
        if (MoveText.endsWith(SymbolLengthSorted[i])) {
            return {move: MoveText.slice(0,-SymbolLengthSorted[i].length), symbol: SymbolLengthSorted[i]};
        }
    }
    return {move: MoveText, symbol: null};
}

var IsBracketEnclosureCorrect=function(Text)
{
...
var PreprocessPGN=function(MovesText)
{
...
export var ParsePGNMovesToMoveTree=function(MovesText, Variant, IsFischerRandom, InitialFEN, Notation, FFishJSLibrary)
{
    if (typeof MovesText!="string" || typeof Variant!="string" || typeof IsFischerRandom!="boolean" || typeof InitialFEN!="string" || Notation==null || FFishJSLibrary==null) {
        throw TypeError();
    }
    ...
    const fen=(InitialFEN==""?FFishJSLibrary.startingFen(Variant):InitialFEN);
    ...
    let i=0;
    let items=PreprocessPGN(MovesText).trim().replace(AllPGNLineCommentMatcher," ").replace(AllLineSplitterMatcher," ").split(WhiteSpaceMatcher);
    let content="";
    let stack=[];
    ...
    for (i=0;i<items.length;i++) {
        ...
        if (content.startsWith("{") && content.endsWith("}")) {
            if (pointer==tree.RootNode || addingvariation) {
                textbefore.push(content.slice(1,-1).replace(AllWhiteSpacePlaceHolderMatcher," "));
            }
            else {
                pointer.Move.TextAfter+=content.slice(1,-1).replace(AllWhiteSpacePlaceHolderMatcher," ");
            }
        }
        ...
        else if (content=="*" || content=="1-0" || content=="0-1" || content=="1/2-1/2") {
            if (i<items.length-1) {
                console.error("Error parsing PGN: Game result can be only added at the end of moves.");
                board.delete();
                return null;
            }
        }
            ...
            if (pointer==tree.RootNode || addingvariation) {
                console.error("Error parsing PGN: A variation line must be added after a move.");
                board.delete();
                return null;
            }
...
{
    AllLineSplitterMatcher=null;
    AllPGNLineCommentMatcher=null;
    AllWhiteSpacePlaceHolderMatcher=null;
    MoveNumberMatcher=null;
    WhiteSpaceMatcher=null;
    LineSplitterReplacer=null;
    SymbolLengthSorted=null;
    ParseSinglePGN=null;
    GetSymbolFromMove=null;
    IsBracketEnclosureCorrect=null;
    PreprocessPGN=null;
    ParsePGNMovesToMoveTree=null;
    Free=null;
}
