...
export class Move {
    constructor(UCIMove,MoverRound,HalfMoveNumber) {
        if (typeof UCIMove!="string" || typeof MoverRound!="number" || typeof HalfMoveNumber!="number") {
            throw TypeError();
        }
        ...
        this.Valid=(moveinfo[0]!==null && moveinfo[1]!==null && moveinfo[2]!==null && moveinfo[3]!==null);
    }
    ...
    static FromString(StringifiedMove) {
      ...
      if (tokens.length==7) {
        let result=new Move(tokens[0],parseInt(tokens[4]),parseInt(tokens[5]));
    ...
    IsPawnPromotion() {
        return (this.MoveInformation[2].charCodeAt(0)>=97 && this.MoveInformation[2].charCodeAt(0)<=122);
    }
    ...
    IsWallGatingOnly() {
        return (this.MoveInformation[0]==this.MoveInformation[1] && this.MoveInformation[3]!="");
    }
    ...
    SetSymbol(SymbolIndex) {
        ...
        if (SymbolIndex==0) {
            ...
            this.Symbol=Symbols[SymbolIndex-1];
        }
    }
    ...
    ToString() {
        return [this.Move,this.Symbol,this.TextBefore,this.TextAfter,this.MoverRound,this.HalfMoveNumber,this.HideSubsequentMoves?"1":"0"].join("\x01");
    }

    ToPGNNote(ForceShowMoveNumber, TotalMoverCount) {
        if (typeof ForceShowMoveNumber!="boolean" || typeof TotalMoverCount!="number") {
            throw TypeError();
        }
    ...
    ToPGNToken(ForceShowMoveNumber, TotalMoverCount) {
      ...
      return {textbefore: textbefore, movenumber: movenumber, ucimove: this.Move, symbol: symbol, textafter: textafter};
    }
}
...
export class MoveTreeNode {
    constructor(MoveObject,PreviousNode,NextNodes) {
        ...
        if (!(PreviousNode instanceof MoveTreeNode) && typeof PreviousNode!="undefined") {
            throw TypeError();
        }
...
class MoveTreeNodeMovePair {
  constructor(TreeNode,MoveObject,IsSplitter) {
    if (!(TreeNode instanceof MoveTreeNode) || !(MoveObject instanceof Move) || typeof IsSplitter!="boolean") {
      throw TypeError();
    }
...
export class MoveTree {
    ...
    SetInitialCondition(InitialMoveNumber,InitialMoverRound,TotalMoverCount) {
      if (typeof InitialMoveNumber!="number" || typeof InitialMoverRound!="number" || typeof TotalMoverCount!="number") {
        throw TypeError();
      }
      if (TotalMoverCount<2) {
        throw RangeError("Total mover count must be greater than or equal to 2.");
      ...
      if (InitialMoverRound<0 || InitialMoverRound>=TotalMoverCount) {
        throw RangeError(`Illegal initial mover round: ${InitialMoverRound}. Min: 0 Max: ${TotalMoverCount-1}`);
      }
      this.InitialMoverRound=InitialMoverRound;
      this.InitialHalfMoveNumber=TotalMoverCount*(InitialMoveNumber-1)+InitialMoverRound+1;
    }
    ...
    AddVariationMove(MoveObject,TreeNodeObject) {
        if (!(MoveObject instanceof Move) || !(TreeNodeObject instanceof MoveTreeNode)) {
            throw TypeError();
        }
        let pointer=this.RootNode;
        this.MainLineMove=[];
        TreeNodeObject.AddVariationNode(new MoveTreeNode(MoveObject,TreeNodeObject,[]));
    ...
    RemoveVariationMove(TreeNodeObject) {
        ...
        if (!TreeNodeObject.IsRootNode()) {
            let pointer=this.RootNode;
            this.MainLineMove=[];
            TreeNodeObject.ParentNode().RemoveNode(TreeNodeObject.RelativePositionInParentNode);
            while (pointer.NextMainNode()) {
              pointer=pointer.NextMainNode();
              this.MainLineMove.push(pointer.Move.Move);
            }
    ...
    SetVariationMoveToMainMove(TreeNodeObject) {
        ...
        if (!TreeNodeObject.IsRootNode()) {
            let pointer=this.RootNode;
            this.MainLineMove=[];
            TreeNodeObject.ParentNode().SetVariationNodeToMainNode(TreeNodeObject.RelativePositionInParentNode);
            while (pointer.NextMainNode()) {
              pointer=pointer.NextMainNode();
              this.MainLineMove.push(pointer.Move.Move);
            }
    ...
    ToPGNTokens(TotalMoverCount) {
      ...
      function Traverse(Node) {
        if (Node instanceof MoveTreeNode) {
          ...
          Node.Move.MoverRound=(depth+initialmoverround-1)%TotalMoverCount;
          if (Node.NextNodes.length>0) {
            result.push(new MoveTreeNodeMovePair(Node.NextNodes[0],Node.NextNodes[0].Move,false));
          }
          else {
            result.push(new MoveTreeNodeMovePair(Node,new Move(")",0,0),true));
          }
          for (i=1;i<Node.NextNodes.length;i++) {
            result.push(new MoveTreeNodeMovePair(Node,new Move("(",0,0),true));
            result.push(new MoveTreeNodeMovePair(Node.NextNodes[i],Node.NextNodes[i].Move,false));
            depth++;
